  ...
  return (
    <article className="max-w-6xl mx-auto px-4 py-12">
      <div className="grid gap-8 md:grid-cols-2 md:gap-12">
        <div className="relative aspect-square w-full max-w-xl bg-[var(--border)]">
          {product.image?.url ? (
            <Image
        ...
        <div>
          <h1
            className="text-3xl font-bold text-[var(--text)] md:text-4xl"
          ...
          {product.tags && product.tags.length > 0 && (
            <ul className="mt-6 flex flex-wrap gap-2" role="list">
              {product.tags.map((tag) => (
                <li
                  key={tag}
                  className="text-sm bg-[var(--border)] text-[var(--text)] px-2 py-1"
                >
                  {tag}
                </li>
              ))}
        ...
        <section className="mt-12 border-t border-[var(--border)] pt-10">
          ...
          <ul className="mt-4 space-y-4" role="list">
            {product.reviews.map((review) => (
              <li
                key={review.id}
                className="border border-[var(--border)] bg-[var(--bg-card)] p-4"
              >
      ...
      <div className="mt-8 flex justify-center border-t border-[var(--border)] pt-5">
